...
ipaSym = [ejm]++tieSym++sSym++xSym++mSym++bSym
...
containsUnreadable ipal = foldl (==) False $ map readable ipal


elemIndexUnreadable :: [IPA] -> [Int]
elemIndexUnreadable ipal = jumperate 0 $ map readable ipal 
  where jumperate _ [] = []
...
data Typing = Consonant | Vowel | Diacritic | Boundary | Sound | None | MixedComplex
  deriving (Eq, Show, Read)
